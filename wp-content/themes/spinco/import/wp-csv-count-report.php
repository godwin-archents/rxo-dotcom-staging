...
function searchForId($id, $array) {
 ...
 function extract_pageXYZ($text){

            $en_text_array = array();
            $html_array = array();
            $html = $text;
            $nodes = extract_tags( $html, 'p|li|div|a' );
            ...
            /*
            $list_html_array = array();
            for ($j = 0; $j < sizeof($last_html_array); $j++) {      
                $last_html = $last_html_array[$j];
                $list_nodes = extract_tags( $last_html, 'li' );
                $list_nodes = array_filter($list_nodes);
                echo "li>>".count($list_nodes)."<br/>";
                if(count($list_nodes)>0){
                    foreach($list_nodes as $link){
                        if($link['contents']!='&nbsp;'){
                            $list_html_array[] = $link['contents'];
                        }            
                    }
                } else {
                    $list_html_array[] = $last_html;
                }    
            }

            $last_html_array = array();
            $last_html_array = $list_html_array;
...
return $en_text_array;        
 ...
 /* Process Start Here */
    ...
    */
    ...
    echo "<hr/>";
    echo iconv( "Windows-1252", "UTF-8", $data[6]);
    echo "<hr/>";
